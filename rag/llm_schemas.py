...
class PolicyDecision(BaseModel):
    ...
    specific_exception_found: str = Field(description="'True' or 'False' along with the specific clause name")
    gap_analysis: str = Field(description="What specific document or clause is missing? Leave empty if none.")
    confidence_score: str = Field(description="'High', 'Medium', or 'Low'")

# Specific sub-schema for the comparison winner
class ComparisonVerdict(BaseModel):
    mathematical_winner: str = Field(description="'Policy A', 'Policy B', 'Tie', or 'Cannot Determine'")
...
class ComparisonResult(BaseModel):
    ...
    comparison_verdict: ComparisonVerdict = Field(description="The final comparison verdict")