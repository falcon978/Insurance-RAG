...
single_policy_decision_template = ChatPromptTemplate.from_messages([
    ("system", 
     ...
     "YOUR STRICT RULES:\n"
     "1. ANTI-BIAS OVERRIDE (CRITICAL): Do not rely on standard insurance industry practices. If a policy explicitly states a condition (e.g., 'no impact'), you must override your prior knowledge and adhere strictly to the provided text.\n"
     "2. NO GUESSING: Do not infer beyond the provided text. If context is missing pillars, flag as CONDITIONAL.\n"
     "3. OUTPUT FORMAT: Output ONLY valid JSON using the exact schema below. No markdown formatting.\n\n"
     "{{\n"
...
single_policy_explainer_template = ChatPromptTemplate.from_messages([
    ("system", 
     ...
     "5. ANTI-BIAS OVERRIDE (CRITICAL): Do not rely on standard insurance industry practices. If a policy explicitly states a condition (e.g., 'no impact'), you must override your prior knowledge and adhere strictly to the provided text.\n\n"
     "FORMATTING RULES:\n"
...
compare_policies_decision_template = ChatPromptTemplate.from_messages([
    ("system", 
     ...
     "ANTI-BIAS OVERRIDE (CRITICAL):\n"
     "Do not rely on standard insurance industry practices. If a policy explicitly states a condition (e.g., 'no impact'), you must override your prior knowledge and adhere strictly to the provided text.\n\n"
     "INDEPENDENCE RULE:\n"
