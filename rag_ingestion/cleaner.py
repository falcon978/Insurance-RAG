...
def clean_markdown_layout(md_text: str) -> str:
    ...
    # 2. Convert SECTION, PART, and ANNEXURE tags to H1 (#)
    cleaned = re.sub(
    ...
    # 3. Demote non-bold text incorrectly tagged as H2 (##)
    cleaned = re.sub(r"^##\s+(?!\*\*)(.*?)$", r"\1", cleaned, flags=re.MULTILINE)

    # 4. Demote orphaned list items marked as H2
    cleaned = re.sub(
        r"^##\s+\*\*([a-z]\.)\*\*\s*$", r"**\1**", cleaned, flags=re.MULTILINE
    )

    # 5. Promote structured lists to H3 (###) for sub-clause chunking
    # 5a. Lettered clauses (Supports: "**a.**", "**(a)**", "**a)**")
    cleaned = re.sub(
        r"(^|\n|[.!?]\s+)\*\*\s*(\(?[a-z][\.\)]\s+[A-Z].*?)\s*\*\*",
        r"\1\n### \2\n",
        cleaned,
    ...
    # 5b. Roman Numeral clauses (Supports: "**iii.**", "**(iii)**", "**iii)**")
    cleaned = re.sub(
        r"(^|\n|[.!?]\s+)\*\*\s*(\(?[ivxIVX]+[\.\)]\s+[A-Z].*?)\s*\*\*",
        r"\1\n### \2\n",
        cleaned,
    ...
    # 5c. Numbered clauses (e.g., "**1.1.2. Clause Name**")
    cleaned = re.sub(
        r"(^|\n|[.!?]\s+)\*\*\s*(\d+(\.\d+)+\.?\s+[A-Z].*?)\s*\*\*",
        r"\1\n### \2\n",
        cleaned,
    ...
    # 5d. Named inline clauses ending with a colon inside asterisks
    # Example: "**Excluded Providers: Code - Excl11:**"
    cleaned = re.sub(
        r"(^|\n)\*\*\s*([^\s*][^*]{2,150}?[:])\s*\*\*", r"\1\n### \2\n", cleaned
    )

    # 5e. Named inline clauses with the colon outside asterisks
    # Example: "**Excluded Providers**: "
    cleaned = re.sub(
        r"(^|\n)\*\*\s*([^\s*][^*]{2,150}?)\s*\*\*\s*:", r"\1\n### \2:\n", cleaned
    )

    # 6. Normalize excess blank lines created during processing
    cleaned = re.sub(r"\n{3,}", "\n\n", cleaned)
